...
Этот гайд помогает применить план из `docs/FEATURES.md`, если у вас есть исходный код и собранный EXE. Он разбит на шаги по настройке окружения, оценке текущей архитектуры и пошаговой интеграции карьеры, тайм-триала и гаража. В конце добавлен краткий чек-лист, чтобы быстрее сверять готовую сборку с планом.
...
- **Лидерборды:** локальное хранилище результатов и адаптер к онлайн-API (ретраи, валидация версии билда). Добавьте фильтры по трассе/машине и защиту от несогласованных данных (чужой билд, моды).
...
- **Каталог косметики:** конфиг с красками, винилами, деталями (ID, редкость, требования к разблокировке). Привяжите цены к экономике кампании и заложите поля под будущие сезоны/ивенты.
- **Применение:** система пресетов/скинов, которая загружает материалы и части при старте гонки; не меняет физику авто. Добавьте fallback для отсутствующих материалов в старых сейвах.
- **Разблокировки:** интеграция с прогрессом (этапы, достижения, задания дня). Добавьте всплывашки/баннеры об открытой косметике и журнал разблокировок.
- **UI:** экран гаража с предпросмотром, категориями и сохранением пресета; подтверждение покупок/выборов, подсказки по редкости/стоимости.
...
## 9. Быстрый чек-лист интеграции
- **Репозиторий:** рабочая ветка от main, CI зелёный, инструкции по сборке обновлены в README/CI-конфиге; EXE воспроизводится по шагам.
- **Данные:** конфиги кампании, тайм-триала и гаража лежат отдельно, валидируются схемами, имеют версионирование и миграции.
- **Код:** сервисы прогресса, лидербордов, кастомизации выделены отдельно; UI-экраны связаны с ними через события/адаптеры, а не прямые обращения к данным.
- **Сборка:** новая EXE покрывает ключевые сценарии — старт кампании, запись/загрузка призрака, выбор и применение пресета в гараже; параметры сборки зафиксированы в CI.
- **Тесты:** юнит- и интеграционные проверки проходят; smoke-тесты по чек-листу прогоняются после сборки.
